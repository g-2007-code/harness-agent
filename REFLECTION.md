...
**subagent-driven-development** 是第二有用的技能。它将 16 个 Task 逐个派发给全新 subagent 实现，每个 subagent 只看到自己那一个 task 的上下文。这带来了两个好处：一是 subagent 不会被无关信息干扰，专注度高；二是我的主上下文不会被 16 个 task 的实现细节污染，可以专注于协调和评审。实际执行中，13 个 subagent 全部一次通过，只有 3 个发现了 PLAN 中的 Windows 兼容性问题并自行修复。

**test-driven-development** 作为硬性要求贯穿全程。每个 task 都是"先写失败测试→验证红色→写最小实现→验证绿色→commit"。61 个测试在最终评审时全部通过，且冷启动验证证明这些测试在陌生 agent 手中也能直接运行。TDD 在 AI 协作下不是阻碍，而是放大器——它给了 subagent 明确的"完成"信号，避免了"实现差不多就交"的模糊地带。
...
在本次项目中，13 个实现 subagent 全部一次通过，无需人工干预。最长的一个 subagent（Task 12 CLI+Keyring）处理了 4 个测试文件和 140 行实现代码，耗时约 3 分钟。subagent 没有偏离主题的情况发生，这归功于 PLAN.md 中每个 task 都有完整的代码——subagent 本质上是在做"转录+测试"，而非"设计+实现"。
...
**最优 task 颗粒度**：一个 task 应该是一个 subagent 在一次会话内能完成的、有独立测试周期的单元。本次 16 个 task 的颗粒度大致在"1-2 个文件 + 3-7 个测试"级别，实践证明这个大小是合适的——足够小到 subagent 不会跑偏，又足够大到有独立的评审价值。
...
2. **假设"TDD 能保证实现质量"**——TDD 能保证机制正确性（mock 测试通过），但不能保证"真实 LLM 会遵循你的格式约定"。我的 61 个测试全部通过，但接入 DeepSeek 后立刻遇到了两个问题（tool_call_id 格式、平台感知），这些是 mock 测试无法覆盖的。
